...
export async function fetch_from_api(endpoint_name, params, callback) {
  ...
  const mock_api_error = new URLSearchParams(window.location.search).get(
    "mock_api_error"
  );
  ...
  try {
    await fetch(api_url, {
      headers: Nominatim_Config.Nominatim_API_Endpoint_Headers || {},
    })
      .then(async (response) => {
        if (
          !(
          ...
          error_store.set(
            `Error de comunicación con ${api_url} (${response.statusText})`
          );
          return undefined;
        }
    ...
    error_store.set(`Error de comunicación con  ${api_url} (${error})`);
    api_request_progress("finish");
  }
...
export async function fetch_content_into_element(url, dom_element) {
  if (!window.location.protocol.match(/^http/)) {
    dom_element.innerHTML =
      `No es posible mostrar información de ${url}. ` +
      "La seguridad del navegador no permite cargar contenido desde URLs de tipo file://";
    return;
  }
  ...
  try {
    await fetch(url)
      ...
      .then((html) => {
        html = html.replace(
          "Nominatim_API_Endpoint",
          generate_nominatim_endpoint_url()
        );
        dom_element.innerHTML = html;
        fetch_content_cache[url] = html;
      });
  } catch (error) {
    dom_element.innerHTML = `Error de comunicación con  ${url} (${error})`;
  }
}
...
function generate_nominatim_api_url(endpoint_name, params) {
  ...
  return (
    generate_nominatim_endpoint_url(endpoint_name) +
    "?" +
